...
def loadDataSet(fileName):
    ...
    for line in fr.readlines():
        ...
        # 将数据转换为浮点类型，便于后面的计算
        fltLine = list(map(float, curLine))  # 映射所有的元素为float(浮点数)类型
...
def randCent(dataMat, k):
    ...
    然后生成0到1之间的随机数并哦通过取值范围和最小值，以便确保随机点在边界之内
    :param dataMat:
    :param k:
    :return:
    """
...
def kMeans(dataMat, k, distMeas=disEclud, createCent=randCent):
    """
    创建k个质心，然后将每个点分配到最近的质心，在重新计算质心
    这个过程重复数次，知道数据点的簇分配结果不再改变为止
    :param dataMat: 数据集
    :param k: 簇的数目
    :param distMeas: 计算距离
    ...
    # 初始化标志变量，用于判断迭代是否继续，如果为true，则继续迭代
    clusterChanged = True
    while clusterChanged:
        ...
        for i in range(m):
            ...
            for j in range(k):
                # 计算数据点到质心的距离
                # 计算距离使用distMeans参数给出的距离公式，默认距离函数是distEclud
                disJI = distMeas(centroids[j, :], dataMat[i, :])
                # 如果距离比minDist(最小距离)还小，更新minDist(最小距离)和最小质心的index(索引)
                if disJI < minDist:
                    minDist = disJI
                    minIndex = j

            # 如果任一点的簇分配结果发生改变，则更新cluterChanged标志
        ...
        for cent in range(k):
            # 通过数据过滤来获得给定簇的所有点
            ptsTnClust = dataMat[nonzero(clusterAssment[:, 0].A == cent)[0]]
            # 计算所有点的平均值，axis=0表示沿矩阵的列方向进行均值计算
            centroids[cent, :] = mean(ptsTnClust, axis=0)
    # 返回所有的类质心与点分配的结果
    return centroids, clusterAssment


def biKmeans(dataMat, k, distMeas=disEclud):
    """
    在给定数据集，所期望的簇数据和距离计算方法下，函数返回聚类结果
    :param dataMat:
    :param k:
    :param distMeas:
    :return:
    ...
    # 对簇不停的进行划分，知道得到想要的簇数目为止
    while (len(centList) < k):
        # 初始最小SSE为无穷大,用于比较划分前后的SSE
        lowestSSE = inf
