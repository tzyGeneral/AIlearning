    ...
    with open(filename, 'r') as fr:
        for line in fr.readlines():
            ...
            for feature in line.split(','):
                # strip()移除字符串头尾的空
                str_f = feature.strip()
                if str_f.isdigit():  # 判断假如是数字
                    # 将数据集的第column列转换为float形式
...
def cross_validation_split(dataset, n_folds):
    """
    将数据集进行重抽样，n_fold份，数据可以重复抽取，每一次list元素都是无重复的
    :param dataset: 原始数据集
    :param n_folds: 数据集dataset分成n_folds份
    :return:
        dataset_split  list集合，存放的是：将数据集进行重抽样 n_folds份，数据集可以重复抽取
    """
    dataset_slpit = list()
    dataset_copy = list(dataset)  # 复制一份dataset，防止dataset的内容被更改
    fold_size = len(dataset) / n_folds
    for i in range(n_folds):
        fold = list()
        while len(fold) < fold_size:
            # 有放回的随机采样，有一些样本被重复采样，从而再训练集中多次出现，有的则从未再训练集中出现，
            # 此为自助采样法。从而保证
            index = randrange(len(dataset_copy))
            # 将对应索引 index 的内容从 dataset_copy 中导出，并将该内容从 dataset_copy 中删除。
            # fold.append(dataset_copy.pop(index))  # 无放回的方式
            fold.append(dataset_copy[index])  # 有放回的方式
        dataset_slpit.append(fold)

    # 由dataset分割出的n_folds个数据构成的列表，为了用于交叉验证
    return dataset_slpit
...
def get_split(dataset, n_features):
    """
    找出分割数据的最优特征，得到最优的特征 index，特征值 row[index]，以及分割完的数据groups（left,right）
    :param dataset:
    :param n_features:
    :return:
    """
