...
path = "./DataSet/"
train = pd.read_csv('./DataSet/train.csv', engine='python', encoding='utf-8')
test = pd.read_csv('./DataSet/test.csv', engine='python', encoding='gbk')
...
class LinearRegression:
    ...
    def fit_normal(self, X_train, y_train):
        ...
        assert X_train.shape[0] == y_train.shape[0], \
            "zhe size of X_train must be equal to the size of y-train"
    ...
    def fit_gd(self, X_train, y_train, eta=0.01, n_iters=1e4):
        ...
        :return: theta 模型参数
        """
        # 根据训练数据集 X_train, y_train, 使用梯度下降法训练Linear Regression模型
        assert X_train.shape[0] == y_train.shape[0], \
            "the size of X_rain must be equal to the size of y_train"

        # 定义损失函数
        def J(theta, X_b, y):
            try:
                return np.sum((X_b.dot(theta)) ** 2) / len(y)
            except:
                return float('inf')
        ...
        X_b = np.hstack([np.ones((len(X_train), 1)), X_train])
        inital_theta = np.zeros(X_b.shape[1])  # 初始化theta
    ...
    def score(self, X_test, y_test):
        # 根据测试数据集，X_test 和 y_test 去顶当前模型的准确度
